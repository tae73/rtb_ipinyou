...
def train_escm2wc_neural(x, win, click, feature_dims, seed):
    cfg = ESCM2WCConfig(feature_dims=feature_dims, embed_dim=EMBED, hidden_dims=HIDDEN,
                        win_hidden_dims=(64, 32), loss_type="dr", dropout=0.2)  # no pos_weight → calibrated
    ...
    for b in _epochs(len(idx), idx, seed * 7 + 2):
        train_step(model, opt, {"x": _jx(x, b), "win": jnp.asarray(win[b].astype("float32")),
                                "click": jnp.asarray(click[b].astype("float32"))})
    return predict_neural(lambda bx: eval_step(model, {"x": bx}).p_ctr, x)


# ---- sweep --------------------------------------------------------------------------------------
import phase_diagram as P   # reuse xfit_isotonic (fit on winners, apply to full) for the recal step
...
def cell(payload, cap, gamma, seed):
    ...
    if cap == "neural":
        ...
        p_deb = train_escm2wc_neural(x_neural, win, click, feature_dims, seed)
    else:
    ...
    p_rec = P.xfit_isotonic(p_bias[w], click[w], p_bias)
    ...
    return {"capacity": cap, "gamma": gamma, "seed": seed, "win_rate": round(float(win.mean()), 3),
            # C1 — optimal-shaded decision-value regret (calibration-robust): does debiasing improve the bid?
            "regret_biased": round(rsh(p_bias), 4), "regret_debiased": round(rsh(p_deb), 4),
            "edge_pp": round((rsh(p_bias) - rsh(p_deb)) * 100, 2),
            # mechanism — pCTR recovery: selection collapses the biased model; debiasing restores scale + spread
            "pctr_true": rec(pctr), "pctr_biased": rec(p_bias), "pctr_debiased": rec(p_deb),
            # TRUTHFUL-bid edge: the project's primary metric (bid = p̂·CPC). Reveals the calibration story —
            # a debiaser that restores spread but overshoots the LEVEL over-bids into losses here.
            "truthful_regret_biased": round(rtr(p_bias), 3), "truthful_regret_debiased": round(rtr(p_deb), 3),
            "truthful_regret_recal": round(rtr(p_rec), 3),
            ...
            "decompose": {"biased": _dec(p_bias, pctr, mprice), "recal": _dec(p_rec, pctr, mprice),
                          "debiased": _dec(p_deb, pctr, mprice), "oracle_surplus": round(s_or, 1)}}
...
def main():
    ...
    print(f"\nNEURAL edge — SHADED (best-case) {summ['shaded_edge_neural_pp']:+.1f}pp  vs  TRUTHFUL (primary) "
          f"{summ['truthful_edge_neural_pp']:+.1f}pp  → reverses by metric: {summ['edge_reverses_by_metric']}")
    print(f"  truthful by γ: {summ['truthful_edge_neural_by_gamma']} (weak-selection helps: {summ['weak_selection_truthful_help']})")
    print(f"  pCTR: true {summ['pctr_recovery_neural']['true_mean']}±{summ['pctr_recovery_neural']['true_std']} → "
          f"biased {summ['pctr_recovery_neural']['biased_mean']}±{summ['pctr_recovery_neural']['biased_std']} → "
          f"debiased {summ['pctr_recovery_neural']['debiased_mean']}±{summ['pctr_recovery_neural']['debiased_std']} (overshoots: {summ['debiasing_overshoots_level']})")
    print(f"  LR/LGB shaded edge {summ['shaded_edge_linear_pp']:+.1f}/{summ['shaded_edge_gbm_pp']:+.1f} | GBM recal-trap {summ['recal_edge_gbm_pp']:+.1f}pp")
    OUT.write_text(json.dumps({
        "_meta": {"design": "iPinYou-GROUNDED semi-synthetic: REAL features + p*(x) fit to REAL winner clicks "
                  ...
                  "finding": "ESCM²-WC restores the selection-collapsed pCTR spread but OVERSHOOTS the level (mean ~1.5x, std ~1.3x truth) "
                             "→ under TRUTHFUL bidding it over-bids into NEGATIVE surplus at strong selection (neural truthful edge -47pp mean); "
                             "the +23.5pp appears only under optimal shading. At WEAK selection it is well-calibrated and helps truthfully (+7.7pp). "
                             "Lesson = the recalibration trap (C2) from the debiaser itself: wrong level → over-bid → loss."},
