...
| **ESCM2-WC(DR) Run AL** | 0.515 | **0.014** | **cfr_lambda=0.2, best** |
...
AUC(ranking)보다 calibration(절대값 정확도)이 더 중요. ESCM2-WC(DR)의 WCTR ECE 0.000003,
WCTR IEB 0.014는 production 배포에 적합한 수준.
...
| **Debiased CTR (unbiased)** | **ESCM2-WC(DR) Run AL** | **DR이 selection bias 제거** |
...
1. **Bid price = base_price × pCTR**: RTB에서 예측 확률은 bid price에 직접 곱해짐.
   ...
   ESCM2-WC(DR)의 WCTR ECE 0.000003은 가격 왜곡을 최소화.
...
4. **최종 추천**: Production 배포에는 **ESCM2-WC(DR)**을 primary model로,
   LGB/LR baseline은 offline evaluation의 benchmark로 유지.
   AUC gap(0.078)은 easy negatives 효과를 감안하면 실질적으로 더 작으며,
