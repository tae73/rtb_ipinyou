...
*Figure 4: 모델별 AUC 및 ECE 비교. LR CTR_all이 all-bids AUC 최고이나, "easy negatives" 효과를 포함한다. (\*All-Bids CTR = WCTR, P(Win)×P(Click|Win)). 모델 비교 3-panel diagnostics (calibration + ROC + score distribution)는 Section 3.3 Figure 10에서 제시한다.*
...
*Figure 10: LGB CTR (won-only, AUC 0.6890) vs LR CTR_all (all-bids, AUC 0.7687) vs ESCM²-WC(DR) (all-bids, AUC 0.6851) 비교 diagnostics. Calibration: ESCM²-WC(DR)이 perfect line에 가장 근접 (IEB 0.073), LGB는 상위 bin에서 overestimation. Score Distribution: 각 모델의 prediction proportion 분포.*
...
*Figure 11: S2→S3 temporal drift. Market price CDF의 우측 이동이 관찰된다 (KS=0.118).*
...
*Figure 12: Per-auction overbidding simulation (재학습 IEB). ESCM2-WC(DR) 7.3%, LR 12.2%, LGB 36.2% bid error.*

![Overbidding Simulation (cumulative)](../results/figures/04_overbidding_simulation.png)
*Figure 13: Cumulative overbidding cost (재학습 IEB). Neural 모델 (360K-600K) vs baseline (LR 976K, LGB 2,896K)의 분기가 핵심이다.*
...
*Figure 14: Calibration의 경제적 가치 (재학습 IEB). Neural debiasing 모델은 모두 near-oracle surplus를 달성. LGB CTR은 Ex1에서 -8% surplus 손실.*
