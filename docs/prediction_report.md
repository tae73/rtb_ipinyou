...
- **ESCM²-WC(DR) 성과**: WCTR AUC 0.6843, IEB 0.014 — ESMM-WC (IEB 1.335) 대비 calibration **95배 개선**. External PS 적용 시 AUC 0.6882 달성
- **Win Tower Dual Purpose**: (a) CTR debiasing을 위한 win propensity 제공, (b) bid shading을 위한 win rate model (AUC ~0.91)
- **Calibration의 경제적 가치**: Oracle (perfect calibration) 대비 ESCM²-WC(DR) (IEB 0.014)는 near-oracle surplus를 달성하는 반면, ESMM-WC (IEB 1.335)는 95배 overbidding cost 발생 및 surplus 14.7% 손실. LR CTR_all (AUC 최고 0.7687)도 IEB 0.122로 8.7배 overbidding, surplus 2.2% 손실. KM CDF 기반 exchange-conditional 분석에서 Ex1 ESMM-WC 42% surplus 손실
...
| **IEB** | Integral Error Bias | 누적 예측 편향 (Σ\|predicted - actual\|) |
...
| ESMM-WC WCTR (Run J) | all bids | — | 0.6905 |
...
| ESMM-WC Win (Run J) | — | 0.6432 |

![AUC ECE Comparison](../results/figures/03_auc_ece_comparison.png)
*Figure 4: 모델별 AUC 및 ECE 비교. LR CTR_all이 all-bids AUC 최고이나, "easy negatives" 효과를 포함한다.*

![Baseline Calibration](../results/figures/03_prediction_baseline_calibration.png)
*Figure 5: Baseline 모델의 calibration curve. LGB Win의 ECE 0.264는 bid shading에 직접 사용 불가능한 수준이다.*
...
| 모델 | Run | WCTR AUC | WCTR IEB | WCTR ECE | CTR AUC (biased) | Win AUC |
|------|-----|----------|----------|----------|------------------|---------|
| LR CTR_all | — | (0.7687)* | — | 0.000028 | — | — |
| LGB CTR (biased) | — | — | — | 0.000514 | 0.6890 | — |
| **ESMM-WC** | **J** | **0.6905** | 1.335 | 0.000308 | 0.6237 | 0.6432 |
| ESCM²-WC (IPW) | Q | 0.6526 | 0.046 | 0.000011 | 0.5292 | 0.6347 |
| **ESCM²-WC (DR)** | **AL** | **0.6843** | **0.014** | **0.000003** | 0.5636 | 0.6403 |
| ESCM²-WC (DR) + ExtPS | AW | **0.6882** | 0.045 | 0.000026 | 0.5713 | 0.6466 |

*\* LR CTR_all의 0.7687은 all-bids 직접 평가이며, WCTR (P(Win)×P(Click|Win)) 형식이 아님. Easy negatives 효과를 포함하므로 직접 비교에 주의가 필요하다.*

**핵심 발견:**
- **ESMM-WC → ESCM²-WC(DR)**: AUC -0.006이나 **IEB 95배 개선** (1.335 → 0.014). DR debiasing이 all-bids CTR의 selection bias를 효과적으로 제거한다.
- **IPW vs DR**: DR (0.6843) >> IPW (0.6526). Overlap 47.8%에서 IPW의 extreme weights가 성능을 악화시킨다.
- **External PS**: AW (0.6882)가 전체 ESCM²-WC best AUC. LGB Win PS (AUC 0.91)의 정확한 importance weights가 DR correction을 개선하나, IEB는 0.014→0.045로 증가 (AUC-Calibration trade-off).

![Debiasing Ablation Ladder](../results/figures/04_debiasing_ablation_ladder.png)
*Figure 6: Debiasing ablation ladder. ESMM-WC에서 ESCM²-WC(DR)으로의 전환에서 calibration이 극적으로 개선된다.*

![AUC vs Calibration Trade-off](../results/figures/04_auc_vs_calibration_tradeoff.png)
*Figure 7: AUC vs Calibration trade-off. Run AL (최고 calibration)과 Run AW (최고 AUC) 간 trade-off가 존재한다.*

**DR Unbiasedness 검증:**

Toy simulation (10K samples, 100 repetitions)에서 DR estimator의 unbiasedness를 검증하였다:
- Naive estimator: bias +0.06 (일관된 과대추정)
- IPW estimator: bias ~0 (correct PS 조건)
- DR estimator: bias ~0, **lower variance** (imputation이 variance를 감소)

![DR Unbiasedness Simulation](../results/figures/04_dr_unbiasedness_simulation.png)
*Figure 8: DR unbiasedness simulation. Naive는 일관된 bias를 보이나, IPW와 DR은 unbiased이며 DR이 lower variance이다.*
...
cfr_lambda=0.2가 imputation tower의 과적합을 억제하면서 DR correction 품질을 유지하는 최적점이다.

![Training Dynamics](../results/figures/04_training_dynamics.png)
*Figure 9: Training dynamics. Win tower의 빠른 수렴과 CTR tower의 느린 학습이 관찰된다.*
...
*Figure 12: S2→S3 temporal drift. Market price CDF의 우측 이동이 관찰된다 (KS=0.118).*

**Target Encoding 실패 분석 (Phase 20):**

ESMM-WC + Target Encoding(10개 TE features) → WCTR AUC 0.6905 → **0.5480 (-0.1424)**:
...
IEB (Integral Error Bias)는 단순한 calibration metric이 아니라, bid pricing에 직접적인 경제적 영향을 미친다. Linear bidding `bid(x) = pCTR(x) × CPC_target`에서 IEB는 곧 bid error 비율이다. 과대추정(IEB > 0)은 systematic overbidding → 높은 가격에 경매 승리 → expected surplus 감소로 이어진다. RTB에서는 AUC (ranking)보다 IEB (calibration)가 revenue에 직결되는 이유가 여기에 있다.

**Overbidding Cost 비교:**

NB04 ablation 결과를 바탕으로, actual CTR = 0.0008 (전체 bids 기준), value_per_click = 10,000 CPM 가정 시 모델별 overbidding cost를 산정한다. Per-bid error = IEB × actual_ctr × value_per_click이다.

| Model | Run | IEB | Per-Bid Error (CPM) | 1M Bids Overbid (CPM) | vs AL |
|-------|-----|-----|--------------------|-----------------------|-------|
| ESCM²-WC(DR) | AL | 0.014 | 0.112 | 112K | 1× |
| ESCM²-WC(DR)+ExtPS | AW | 0.045 | 0.360 | 360K | 3.2× |
| LR CTR_all | — | 0.122 | 0.976 | 976K | **8.7×** |
| LGB CTR (biased) | — | 0.362 | 2.896 | 2,896K | **25.9×** |
| ESMM-WC | J | 1.335 | 10.68 | 10,680K | **95×** |

LR CTR_all은 all-bids AUC 최고 (0.7687)이지만 IEB 0.122로 impression당 0.976 CPM의 overbid를 유발하며, ESCM²-WC(DR)보다 **8.7배** 높은 overbidding cost를 발생시킨다. LGB CTR은 won-only 편향 (IEB 0.362)으로 **25.9배**에 달한다. ESMM-WC의 IEB 1.335는 impression당 10.68 CPM의 systematic overbid를 유발하며, 1M bids 규모에서 ESCM²-WC(DR)보다 **95배** 높은 overbidding cost를 발생시킨다. 이는 **AUC best ≠ bidding best**임을 명확히 보여준다: LR CTR_all의 AUC 0.7687은 ranking에서 최고이나, calibration 부족으로 bid pricing에서는 ESCM²-WC(DR)에 크게 뒤진다.

![Overbidding Simulation (per-auction)](../results/figures/03_overbidding_simulation.png)
*Figure 13: Per-auction overbidding simulation. IEB가 높은 모델일수록 bid error가 체계적으로 누적된다.*

![Overbidding Simulation (cumulative)](../results/figures/04_overbidding_simulation.png)
*Figure 14: Cumulative overbidding cost. ESMM-WC의 누적 초과 지불이 ESCM²-WC(DR) 대비 95배에 달한다.*
...
| Model | V(x) (CPM) | Optimal Bid b* (CPM) | Shade Factor | Expected Surplus (CPM) | Loss vs Oracle |
|-------|-----------|---------------------|-------------|----------------------|----------------|
| Oracle (perfect) | 160.0 | 80 | 50.1% | 11.24 | — |
| ESCM²-WC(DR) AL | 162.2 | 80 | 49.4% | 11.24 | ~0% |
| ESCM²-WC(DR)+ExtPS AW | 167.2 | 80 | 47.9% | 11.24 | ~0% |
| LR CTR_all | 179.5 | 89 | 49.6% | 11.00 | **-2.2%** |
| LGB CTR (biased) | 217.9 | 89 | 40.9% | 11.00 | **-2.2%** |
| ESMM-WC J | 373.6 | 101 | 27.1% | 9.59 | **-14.7%** |

ESCM²-WC(DR) Run AL의 낮은 IEB (0.014)는 V(x)를 true value에 근접하게 추정하여 near-oracle surplus를 달성한다 (KM CDF의 discrete grid 해상도 내에서 차이 미미). LR CTR_all (IEB 0.122)과 LGB CTR (IEB 0.362)은 각각 V(x)를 12%/36% 과대추정하여 optimal bid가 89 CPM으로 상승하고, surplus가 2.2% 감소한다. ESMM-WC Run J는 V(x)를 2.3배 과대추정하여 optimal bid가 101 CPM까지 상승하고, expected surplus가 14.7% 감소한다. 주목할 점은 LR CTR_all이 AUC에서 최고 (0.7687)임에도 surplus에서는 ESCM²-WC(DR)에 뒤진다는 것으로, ranking 능력과 bidding 수익성이 별개의 축임을 보여준다.

**Exchange-Conditional Surplus 비교:**

Exchange-conditional CDF를 적용하면 miscalibration의 경제적 영향이 더욱 극대화된다:

| Exchange | F(300) | Oracle | AL | LR CTR_all | LGB CTR | J |
|----------|--------|--------|-----|-----------|---------|-----|
| Ex1 | 68.8% | 25.28 | 25.28 (~0%) | 24.76 (-2.1%) | 23.22 (-8.2%) | 14.72 (-42%) |
| Ex2 | 29.1% | 14.82 | 14.82 (~0%) | 14.80 (-0.2%) | 14.58 (-1.6%) | 10.83 (-27%) |
| Ex3 | 11.9% | 8.09 | 8.09 (~0%) | 8.09 (~0%) | 8.09 (~0%) | 8.08 (~0%) |

Win rate가 높은 Ex1에서 miscalibration의 영향이 가장 크다: ESMM-WC의 과대추정된 V(x)가 공격적 입찰로 이어져 surplus의 42%를 상실한다. LGB CTR도 Ex1에서 8.2% surplus 손실을 보이며, LR CTR_all은 2.1% 손실로 baseline 중에서는 양호하나 ESCM²-WC(DR)의 ~0% 손실에는 미치지 못한다. Ex3에서는 시장이 매우 경쟁적이어서 (F(300)=12%) 모든 모델의 optimal bid가 유사하다.

![Calibration Economic Value](../results/figures/05_calibration_economic_value.png)
*Figure 15: Calibration의 경제적 가치. (Left) Overall optimal bid surplus — 5개 모델 비교, ESMM-WC의 miscalibration으로 14.7% surplus 손실. (Right) Exchange-conditional 분석 — Ex1에서 LGB 8.2%, ESMM-WC 42% surplus 손실.*

**Production 모델 선택 근거:**

LR CTR_all은 all-bids AUC 0.7687로 ranking 최고이나, IEB 0.122로 bidding에서는 8.7× overbidding cost와 2.2% surplus 손실이 발생한다. LGB CTR은 won-only 편향 (IEB 0.362)으로 25.9× overbidding cost와 Ex1에서 8.2% surplus 손실을 보인다. ESMM-WC Run J는 WCTR AUC에서 +0.006 우위를 보이지만, 95× overbidding cost와 14.7% surplus 손실을 감안하면 bid pricing 용도에 부적합하다. ESCM²-WC(DR) Run AL은 calibration best (IEB 0.014)로 optimal bid pricing에 최적이며, Run AW는 AUC best (0.6882)로 ad ranking에 최적이다. 두 모델의 A/B test로 revenue impact를 직접 비교할 것을 권장한다.
...
**2. DR이 Calibration에서 압도적 우위**

ESCM²-WC(DR) Run AL의 WCTR IEB 0.014는 ESMM-WC Run J의 1.335 대비 **95배 개선**이다. ESMM-WC는 CTR tower 자체는 well-calibrated (IEB 0.005)하나, P(Win)×P(Click|Win) product에서 error가 누적된다. DR의 explicit correction이 이 product bias를 효과적으로 보정한다. RTB production에서 bid price = base_price × pCTR이므로, calibration은 AUC보다 중요하다. Section 3.5의 overbidding simulation (95× cost 차이)과 KM CDF 기반 surplus 분석 (14.7% 손실)이 이를 정량적으로 확인한다.

**3. Win Tower Dual Purpose**

Win Tower는 (a) CTR debiasing을 위한 propensity score와 (b) bid shading을 위한 win rate model의 이중 역할을 수행한다. External LGB Win PS (AUC 0.91)를 활용하면 DR correction 품질이 추가로 개선된다 (Run AW WCTR AUC 0.6882). 이 dual purpose 설계는 모델 serving 시 추가 네트워크 호출 없이 debiasing과 bid optimization을 동시에 수행할 수 있게 한다.
...
| 용도 | 추천 모델 | 근거 |
|------|----------|------|
| **Bid Pricing** (calibration 중시) | ESCM²-WC(DR) Run AL | WCTR IEB 0.014, ECE 0.000003 |
| **Ad Ranking** (AUC 중시) | ESCM²-WC(DR)+ExtPS Run AW | WCTR AUC 0.6882 |
| **All-bids Ranking** (easy negatives 포함) | LR CTR_all | Test AUC 0.7687 (단, calibration 해석 주의) |
| **Winners-only Ranking** | LGB CTR | Test AUC 0.6890 (biased) |

Production 환경에서는 두 모델의 A/B test로 revenue impact를 직접 비교하는 것을 권장한다.
...
**5. AUC Gap vs LR**
ESCM²-WC(DR)의 WCTR AUC (0.6882)와 LR CTR_all (0.7687) 간 gap이 존재한다. 이 gap의 일부는 LR의 easy negatives 효과에 기인하나, neural model의 temporal drift 취약성도 기여한다.
...
2. **Debiasing Ablation**: Biased baseline → ESMM-WC → ESCM²-WC(IPW) → ESCM²-WC(DR) 순차적 비교를 통해 DR debiasing이 calibration을 95배 개선 (IEB 1.335 → 0.014)함을 보였다. 이는 RTB의 positivity violation (ESS 9.66%) 하에서 DR의 doubly robust 특성이 핵심적으로 작용한 결과이다.

3. **AUC-Calibration Trade-off 규명**: Ranking 능력과 확률 calibration이 독립적인 평가 축임을 실증하고, production 용도별 모델 선택 가이드를 제시하였다 (bid pricing → Run AL, ad ranking → Run AW).
...
5. **Calibration → Bid Optimization 인과 체인**: IEB 95배 calibration 차이 (ESMM-WC 1.335 vs ESCM²-WC(DR) 0.014) → overbidding cost 95배. KM CDF 기반 exchange-conditional optimal bid에서 calibrated model (Run AL)이 oracle (perfect calibration) 대비 near-oracle surplus를 달성하는 반면, miscalibrated model (Run J)은 Ex1에서 42% surplus를 상실한다. 이 인과 체인 — debiasing → calibration → bid pricing → revenue — 이 본 프로젝트의 핵심 결론이다.

**Production 추천**: Calibration이 중요한 bid pricing에는 ESCM²-WC(DR) Run AL (IEB 0.014, 95× overbidding cost 절감, near-oracle surplus)을, ranking이 중요한 ad selection에는 Run AW (WCTR AUC 0.6882)를 권장한다. Exchange-conditional bid shading과 temporal adaptation을 결합하면 iPinYou의 76% overpayment을 대폭 줄일 수 있다.
...
20단계 성능 튜닝의 요약이다. 상세 로그는 `docs/performance_tuning.md` 참조.
