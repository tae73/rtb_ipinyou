...
- **★ Neural anchor — 실 feature + 실 모델 (cautionary).** **iPinYou-grounded** semi-synthetic(실 feature +
  실 **ESCM²-WC**)에서, win-selection bias가 biased 신경망 pCTR을 **붕괴**(분산 0.153 → 0.056)시키고
  ESCM²-WC가 분산을 복원(→ 0.198)하나 **레벨을 overshoot**(mean → 0.127). 프로젝트의 **primary truthful-bid
  metric에선 과입찰로 손실**(edge **−47pp**, 강selection서 음의 surplus); **+23.5pp**는 *오직* optimal
  bid-shading서만. 정직한 교훈: **디바이어싱은 ranking은 고치나 입찰엔 calibration 필요** — 디바이어 자체에서
  나온 C2 trap. (첫 "+23.5pp 승리" 헤드라인을 적대 리뷰가 적발.)
...
| ★ | **Neural anchor** — 실 feature + 실 **ESCM²-WC** (cautionary) | ESCM²-WC가 붕괴된 분산 복원(0.056→0.198)하나 레벨 **overshoot** → **truthful** edge **−47pp**(과입찰); +23.5pp는 optimal shading서만 | `witnesses/neural_anchor.json` | **cautionary** (metric 의존) |
...
## ★ Neural anchor — 실 iPinYou feature + 실 ESCM²-WC (cautionary)
...
(Flax, [`old/src/`](old/src)) — {LR, LGB, **신경망 ESCM²-WC**}로 재실행.

<p align="center">
  <img src="witnesses/figures/fig_neural_anchor.png" alt="실 feature에서 ESCM²-WC가 붕괴된 pCTR 분산을 복원하나 레벨 overshoot; truthful bidding서 강selection시 과입찰로 손실(edge -47pp), +23.5pp는 optimal bid-shading서만" width="900">
</p>

> **적대-정직 노트.** 첫 패스는 "**+23.5pp** 신경망 디바이어싱 edge"를 헤드라인으로 썼다. 적대 리뷰가
> 이것이 **metric 의존**임을 적발 — *optimal bid-shading*서만 성립; 프로젝트의 **primary truthful-bid metric에선
> 결과가 역전**(평균 **−47pp**). 둘 다 재보고; shading metric은 ranking이 아니라 *spread*를 보상해 over-spread
> 모델을 flatter한다. 그래서 **cautionary**로 제시.

- **메커니즘 — 붕괴 후 OVERSHOOT.** win-selection bias가 biased 신경망 pCTR을 **붕괴**(mean 0.083 → 0.050,
  분산 0.153 → 0.056). 실 ESCM²-WC는 분산을 복원(→ 0.198)하나 **레벨을 overshoot**(mean → 0.127, ≈1.5× true).
- **Metric 역전.** **truthful bidding**(primary): 신경망 edge **−47pp** 평균 — γ별 **+7.7**(약selection,
  진짜 도움) → −29 → **−120**(강selection: 음의 surplus로 과입찰, γ=1.2서 −1.3M). **optimal shading**(best-case):
  +23.5pp. 복원된 spread는 shading이 과입찰을 rescale해야 비로소 이득이 된다.
- **정직한 교훈.** 실 ESCM²-WC는 **ranking은 고치나 calibration을 overshoot**해 truthful bidding서 과입찰 —
  **디바이어 자체에서 나온 recalibration trap(C2)**: 잘못된 *레벨* → 과입찰 → 손실. ranking 복원하나 **입찰엔 calibration 필요**.
- **C2 (실 feature).** recal trap은 **GBM서 재현**(−14.3pp; 강selection −36~−39pp)되나 신경망(+2.8)선 아님
  (biased under-predict → recal 도움). capacity 의존.

> 정직: iPinYou-**grounded** semi-synthetic — p\*(x)는 fitted surrogate, market은 실 payprice로 fit, selection은
> 합성; decision-value는 실 iPinYou서 측정 불가(천장). **truthful surplus가 primary; optimal-shading은
> best-case이며 spread를 보상 — 대신이 아니라 함께 보고.** 상세 [`methods.md`](methods.md) §6.
