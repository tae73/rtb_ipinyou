...
- **★ Neural anchor — 실 feature + 실 모델.** **iPinYou-grounded** semi-synthetic(실 feature + 실 **ESCM²-WC**)에서
  ...
  truthful bidding을 진짜로 도움: +7.5pp**(γ 전반 일관), selection-aware **IPW calibration** 시 더(**+11pp**,
  극단 selection 제외). LR/LGB는 도움 안 됨(−4.8/−1.6). 2차 자기교정 — 과입찰은 data-contract 버그였지 근본
  miscalibration이 아니었다.
...
| ★ | **Neural anchor** — 실 feature + 실 **ESCM²-WC** | −47pp 과입찰은 **censoring 버그**였음; 수정(`click·win`) 후 ESCM²-WC가 truthful bidding **+7.5pp** 도움(selection-aware calibration 시 **+11**) | `witnesses/neural_anchor.json` | **modest positive** (corrected) |
...
- **수정 — click 검열(`click·win`).** 과입찰 소멸: 디바이어 pCTR이 더는 overshoot 안 함(mean 0.083 → 0.065),
  **신경망 truthful edge = +7.5pp**(−47.2였음), 6개 cell 모두 양수(γ별 6.8 / 7.8 / 7.8). conservative bidding이
  아니라 진짜 recovery — 디바이어가 biased보다 **더 높게 입찰**하고 surplus를 **더** 얻음. (*주의:* n=2 seed/γ →
  **부호**만 신뢰, magnitude는 under-powered.)
- **calibration으로 더 개선?** 약간, 정직한 반전과 함께: naive isotonic-on-winners는 selection bias를 *재도입*하므로
  **selection-aware IPW-weighted isotonic** 사용 → **+10.9pp**. 단 selection-aware *우위는 미발현* — naive가
  동률(+11.0)이고 극단 selection서 오히려 앞섬(γ=1.2: 9.1 vs 7.4, ESS↓). **무거운 일은 censoring 수정이 했다.**
  LR/LGB는 음수(−4.8 / −1.6) — 신경망 디바이어만 도움.
- **"문제/고칠 수 있나"의 정직한 답:** 과입찰은 대부분 **data-contract 버그**였지 근본 miscalibration이 아니었다.
  수정 후 ESCM²-WC가 도움(+7.5pp), calibration이 약간 더(극단 selection 제외).
