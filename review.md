...
- **방향교정 #2(censoring 버그):** "−47pp 과입찰"의 *원인*을 파보니 대부분 **testbed 배선 버그**였다 —
  ...
  p_ctr 부풀림(mean→0.127). **수정(`click·win`):** overshoot 소멸(mean→0.065), **neural truthful edge +7.5pp**
  (γ 일관 6.8/7.8/7.8). 사전 수치는 `_meta.frozen_prefix_result`에 freeze(감사용).
- **calibration(질문의 직관):** selection-aware **IPW-weighted isotonic**(winners를 1/P(win|x)로 재가중 →
  marginal로 calibrate; naive는 winners-low-pCTR 편향 재도입)로 **+10.9pp**. **단 IPW의 selection-aware 우위는
  미발현** — naive가 동률(+11.0)·non-monotone·극단 selection서 오히려 앞섬(γ=1.2: naive 9.1 vs IPW 7.4; ESS
  mean 0.74→γ1.2 ~0.63). 무거운 일은 censoring 수정이 했고 calibration은 작은 lift.
- **honest 결론:** 과입찰은 **data-contract 버그**였지 근본 miscalibration 아님 — 수정 후 실 ESCM²-WC가 truthful
  bidding을 진짜로 도움(+7.5pp; **n=2 seed/γ라 부호만 신뢰**). LR/LGB는 도움 안 됨(−4.8/−1.6). semi-synthetic·천장 동일.
