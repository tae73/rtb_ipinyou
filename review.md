...
## 5.5 Neural anchor — done, **modest positive** (실 feature + 실 모델; 2회 교정)
...
- **방향교정 #1(metric):** 첫 "+23.5pp"는 optimal-shading서만 → truthful metric에선 −47pp(적대 리뷰 적발).
- **방향교정 #2(censoring 버그):** "−47pp 과입찰"의 *원인*을 파보니 대부분 **testbed 배선 버그**였다 —
  joint-BCE `BCE(p_win·p_ctr, click)`은 **censored click**(loser=0) 기대인데 uncensored 합성 click을 먹임 →
  p_ctr 부풀림(mean→0.127). **수정(`click·win`):** overshoot 소멸(mean→0.064), **neural truthful edge +7.2pp**
  (γ 양수 5.1/7.6/8.7). 사전 수치는 `_meta.frozen_prefix_result`에 freeze(감사용).
- **calibration 2×2 테스트(질문의 직관, 가정 없이 검정):** {biased,debiased}×{naive,IPW}. 일반 calibration은
  도움(+7.2→+11.1IPW/+11.2naive). **selection-aware IPW는 *약selection(overlap 좋을 때)*에만 naive를 이김**
  (γ=0.4 ESS 0.87: +1.2pp; 레벨을 marginal로 올림 0.050→0.060 vs naive 0.054 — HT가 작동). **강selection**
  (ESS 0.63)선 고분산 가중이 over-lift → over-bid → **짐**(γ=1.2: −6.4pp); net naive 무승부/우위(biased −1.8, debiased −0.1).
  **방향교정:** 앞서 "IPW≈naive는 already-debiased라서"라던 추정은 틀렸고, 실제 driver는 **overlap(ESS)**.
  교훈=**calibration 정확도 ≠ 입찰 가치**(레벨-정확 보정이 margin서 over-bid = C2 trap의 calibration 버전).
- **honest 결론:** 과입찰은 **data-contract 버그**였지 근본 miscalibration 아님 — 수정 후 실 ESCM²-WC가 truthful
  bidding을 진짜로 도움(+7.2pp; **n=2 seed/γ+p* non-determinism라 부호/방향만 신뢰**). LR/LGB는 도움 안 됨(−4.8/−1.6).
